...
import { allProducts, BRANDS, COLORS } from "@/lib/data";

const VALID_SLUGS = ["men", "women", "unisex", "premium"];
...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const defaultGender = slug === "men" ? "Men" : slug === "women" ? "Women" : slug === "unisex" ? "Unisex" : null;
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (defaultGender) {
      result = result.filter(p => p.gender === defaultGender);
    }
    if (slug === "premium") {
        result = result.filter(p => p.price >= 450);
    }
    ...
    return result;
  }, [selectedBrands, selectedColors, sortBy, defaultGender, slug]);

  const displayTitle = slug === "men" ? "Men's Collection" : 
                       slug === "women" ? "Women's Collection" : 
                       slug === "premium" ? "Premium Selection" : "Unisex Collection";
    ...
    <div className="min-h-screen bg-black pt-32 pb-24">
      <div className="px-6 md:px-12 max-w-[1400px] mx-auto w-full">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 border-b border-white/10 pb-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-serif text-white mb-4">{displayTitle}</h1>
            <p className="text-gray-400 font-light max-w-xl">
              Curated luxury styles tailored exactly for your fit and aesthetic. Handpicked from world-class designers.
            </p>
          </div>
